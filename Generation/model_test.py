...
class Model(object):
    ...
    def draw_correspondense(self):
        ...
        for i in range(sample_num):
            ...
            color = np.zeros((number+1, self.opts.np, 3)) #3
            ...
            print(plot_name)
            #print("pcds before plot_pcd_three_views")
            print(pcds)
            pcd_cloud = o3d.geometry.PointCloud()
            pcd_cloud.points = o3d.utility.Vector3dVector(pcds[2])
            o3d.io.write_point_cloud(plot_name +"output.ply" , pcd_cloud, write_ascii=False, compressed=False, print_progress=True)
            print("save")
            f = open('pcds_array.txt', 'w')
            pcds.tofile(f)
            with np.printoptions(threshold=np.inf):
                f = open('pcds_array_test.txt', 'w')
                print(pcds, file = f)
